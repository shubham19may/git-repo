...
public class Today_performance_history_activity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) 
	{
		...
		change = new Change_in_ani_calculator(1, this);
		try {
		...
		all_days.setText(""+get_days_played());
		all_games = (TextView)findViewById(R.id.all_games);
		all_games.setText(""+games_played());
	}
	...
	public int get_days_played()
	{
		...
		for(int i =1 ; i<=18;i++)
		{
			...
			for(int j = 1;j<=scr_mgr.get_no_of_rows();j++)
			{
				...
				if(todays_date_s.matches(scr_mgr.get_date_from_id(j)))
				{
					scr_mgr.close();
					flag =1;
					break;
